...
class ProjectNavigator(ctk.CTkFrame):
    ...
    Features:
    - Panel-based navigation (World Bible, Script, Storyboard, Gallery, Editor)
    - Icon-only buttons with frames for visual definition
    - Neon green navigation highlighting
    - Workflow progress tracking
    """

    # Navigation panels - each leads to a dedicated view
    # Format: (panel_id, icon, label, description)
    NAV_PANELS = [
        ("world_bible", "🌍", "World", "Characters, locations, props"),
        ("script", "📜", "Script", "View and edit story scenes"),
        ("storyboard", "🎬", "Board", "Visual storyboard frames"),
        ("gallery", "🖼️", "Gallery", "Project images and archives"),
    ...
    def _setup_ui(self) -> None:
        ...
        # Navigation panels section - icon buttons with frames
        nav_frame = ctk.CTkFrame(self, fg_color="transparent")
        nav_frame.pack(fill="x", padx=theme.spacing.md, pady=(theme.spacing.md, theme.spacing.md))

        # Create a horizontal row of icon buttons
        icon_row = ctk.CTkFrame(nav_frame, fg_color="transparent")
        icon_row.pack(fill="x")

        for panel_id, icon, label, panel_desc in self.NAV_PANELS:
            # Editor is "coming soon" - disable it
            is_disabled = panel_id == "editor"

            # Create a frame container for each icon button
            ...
            btn_container.pack(side="left", padx=2, pady=2)

            # Icon button inside the frame
            btn = ctk.CTkButton(
                btn_container,
                text=icon,
                width=48,
                ...
                text_color=theme.colors.text_primary if not is_disabled else theme.colors.text_muted,
                font=ctk.CTkFont(size=20)
            )
            btn.pack(padx=2, pady=2)

            # Store both button and container for highlighting
            self._workflow_buttons[panel_id] = btn
            if not hasattr(self, '_workflow_containers'):
                self._workflow_containers = {}
            self._workflow_containers[panel_id] = btn_container
    ...
    def _update_workflow_highlighting(self) -> None:
        """Update panel button highlighting - current gets green highlight on container frame."""
        containers = getattr(self, '_workflow_containers', {})

        for panel_id, btn in self._workflow_buttons.items():
            container = containers.get(panel_id)
            ...
            if panel_id == self._current_workflow_step:
                # Current selection - neon green border on container, green icon
                if container:
                    container.configure(
                        border_width=2,
                ...
                # Completed panel - subtle green border on container
                if container:
                    container.configure(
                        border_width=1,
                ...
                # Inactive panel - default border on container
                if container:
                    container.configure(
                        border_width=1,
                        border_color=theme.colors.border,
