...
- ❌ `Mei` → ✅ `[CHAR_MEI]`
- ❌ `[Mei]` → ✅ `[CHAR_MEI]`
- ❌ `CHAR_MEI` (no brackets) → ✅ `[CHAR_MEI]`
- ❌ `[char_mei]` (lowercase) → ✅ `[CHAR_MEI]`
- ❌ `[CHARACTER_MEI]` (wrong prefix) → ✅ `[CHAR_MEI]`

### Scene.Frame.Camera Mistakes
- ❌ `{frame_1.2}` (old format) → ✅ `[1.2.cA]`
- ❌ `1.2.A` (missing 'c') → ✅ `1.2.cA`
- ❌ `[1.2.ca]` (lowercase) → ✅ `[1.2.cA]`
- ❌ `Scene 1` → ✅ `## Scene 1:`

### Prompt Instruction Mistakes
- ❌ "Use tags like [TAG_NAME]" (placeholder) → ✅ "Use tags like [CHAR_MEI], [LOC_PALACE]" (examples)
- ❌ "Format as [CHAR_X]" (ambiguous) → ✅ "Format as [CHAR_FIRSTNAME] e.g., [CHAR_MEI]"
...
2. **Scene.Frame.Camera Instructions**
...
## 🛡️ NOTATION CONSISTENCY MAINTENANCE RULES
...
- ❌ Never use: `[TAG_NAME]`, `[CHAR_X]`, `[LOC_NAME]`
- ✅ Always use: `[CHAR_MEI]`, `[LOC_PALACE]`, `[PROP_SWORD]`
...
1. Move to `.archive/deprecated/` with deprecation notice
2. Add entry to `DEPRECATION_PLAN.md`
3. Add deprecation warning to any remaining functions
4. Update this file's KEY FILES REFERENCE section
...
```
Project Selection → set_project_path() → set_mode() → _show_{mode}()
                                                           ↓
                                                    Load data files
                                                           ↓
                                                    Parse content
...
| Parser | Input | Output |
|--------|-------|--------|
| `ScriptParser` | `script.md` content | List of scene dicts with tags (scene-only, no beats) |
...
## 🔧 UI DEVELOPMENT PROCESSES

### Adding a New Workspace Panel

1. Add mode to `WorkspaceMode` enum in `main_workspace.py`
2. Add case in `set_mode()` method
3. Create `_show_{mode}()` method following existing patterns:
   - Get project path from `_current_content`
   - Handle empty state if no project
   - Load data files
   - Parse content using appropriate parser
   - Render UI cards
4. Add refresh method if needed: `refresh_{mode}()`
5. Update navigator panel to include new mode option
...
```
[visual_style_mapped]. [style_notes]. Lighting: [lighting]
```

Example:
```
photorealistic live-action cinematography. Dark cinematic styling with muted tones. Lighting: Chiaroscuro with low-key, volumetric lighting
```

Note: The `vibe` field is intentionally excluded from the style suffix. It remains in world_config.json for other uses but is not appended to image prompts.
...
1. **Never read style from style_guide.md or pitch.md** - these are deprecated for style data
2. **Always use `ImageHandler.get_style_suffix()`** - never manually build style suffix
3. **All style edits save to world_config.json only** - no backward compatibility writes
4. **Writer pipeline generates lighting/vibe** - saved to world_config.json via writer_dialog.py
...
## 🤖 OMNIMIND AUTONOMOUS PIPELINE EXECUTION
...
| Tool | Description |
|------|-------------|
| `run_e2e_pipeline` | Execute complete Writer → Director → References → Storyboard pipeline |
...
1. **Writer** - Generates `scripts/script.md` from `pitch.md` and `world_config.json`
2. **Director** - Processes script to create `visual_script.json` with scene.frame.camera notation
3. **References** - Extracts tags and generates reference images in `references/{TAG}/`
4. **Storyboard** - Generates final images in `storyboard_output/` using Seedream 4.5
